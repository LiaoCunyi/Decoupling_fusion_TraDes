...
class opts(object):
  def __init__(self):
    ...
    # 是否需要冻结训练
    self.parser.add_argument('--backbone_freeze_or_unfreeze', default='False',
                             help='True freeze ,False unfreeze')
    # 冻结2Dhead或者seg,让它们分开训练
    self.parser.add_argument('--backbone_2Dhead_or_seg', default='True',help='True freeze seg and Train 2Dhead,False freeze 2Dhead and Train seg')
    # self.parser.add_argument('--backbone_2Dhead_and_seg', default='False',help='True Train seg and 2Dhead')
    self.parser.add_argument('--training_seg', default=True)
    ...
    self.parser.add_argument('--training_att'
                             'ention', default=True)

    self.parser.add_argument('--attention_swith',default='ECA',help='swith_attention:ECA,DA,self')

    # loss
    self.parser.add_argument('--loss_switch', default='DWA', help='loss_attention:DWA,None,Uncertainty_Weighting,Grad,DWA_change')

    # head_merge
    self.parser.add_argument('--head_switch', default='merge-attention', help='head_switch:None,+,cat,attention,merge-attention')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.loss_switch == 'Uncertainty_Weighting':
        opt.awl = AutomaticWeightedLoss(6)
