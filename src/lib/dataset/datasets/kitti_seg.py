...
class KITTISeg(GenericDataset):
    ...
    def __init__(self, opt, split):
        data_dir = os.path.join(opt.data_dir, 'kitti_seg')
        ...
        ann_path = os.path.join(
            data_dir, 'annotations', 'tracking_{}.json'.format(
                ann_file_))
        self.images = None
        super(KITTISeg, self).__init__(opt, split, ann_path, img_dir)
    ...
    def save_results(self, results, save_dir):
        results_dir = os.path.join(save_dir, 'results_kitti_seg')
