...
class KITTITracking(GenericDataset):
  ...
  def __init__(self, opt, split):
    ...
    # img_dir = os.path.join(
    #   data_dir, 'real_data_tracking_image_2', '{}ing'.format(split_), 'image_02_new')
  ...
  def run_eval(self, results, save_dir):
    ...
    os.system('python3 tools/eval_kitti_track/evaluate_tracking.py ' + \
  ...
  def statistics(self,results):
    ...
    for result in results:
      for i in range(len(results[result])):
        object = results[result][i]
        total = total + 1
        xiao = (object['bbox'][2]-object['bbox'][0]) * (object['bbox'][3]-object['bbox'][1])
        if xiao < 4000:
          xiaomubiao = 1 + xiaomubiao
